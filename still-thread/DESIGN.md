...
### 最終待機（初版: 20秒フル待機で自動クリア。次項で上書き済み）

- `STAY_TARGET`: 10000→20000（20秒）。
- タイマー開始条件・一時停止・可視FBの立ち上がり緩和・GA4追加の設計はいずれも下記「15秒＋ダイアログ化」にそのまま引き継がれている（数値と終着点のみ変更）。

---

## v3.1 最終待機の再改訂（2026-07-12・3回目: 15秒＋二択ダイアログ）

20秒フル待機での自動クリアを廃止し、**15秒の静止で「見送るか、見に行くか」を尋ねるダイアログ**に変更した。

- `STAY_TARGET`: 20000→**15000**（15秒）。ダイアログが現れるまでの静止時間。
- **タイマー開始条件（変更なし）**: `enterChoice()`でビートC（c1即時＋c2）と「次スレへ」ボタンが表示し終わるまで（`STAY_READY_DELAY=2500ms`）は`state.stayTimerStarted`が`false`のままカウントしない。c2表示と同時に`stayTimerStarted=true`にして`stay_begin`イベントを発火。
- **一時停止（変更なし）**: `state.stayMs`の加算条件は`state.still && !state.pressing`。スクロール・押下中は加算を止めるだけでゼロに戻さない。静止に戻れば続きから再開。
- **可視FBの立ち上がり（変更なし）**: `STAY_FADE_START=5000ms`。開始5秒間はスレの薄れ・水流減衰・囮の褪色がほぼ見えないようにし、初見が囮を押すか待つか検討する時間を確保。
- **ダイアログ**: `STAY_TARGET`到達時、`doClear()`を直接呼ばず`showLeaveDialog()`を呼ぶ。新設の`.leave-dialog`（`.overlay`は使わない小さなカード、`top:44%`＝シミ`--safe-shimi:32%`と囮`#decoy top:58%`の間）に本文「え、次のスレ見ないの？」と「見ない」「見る」の2ボタン（各`min-height:44px`、`:focus-visible`対応）を表示。表示後は`state.dialogShown=true`で選択フェーズの通常の加算・可視FB更新を凍結する。
- **「見ない」→クリア（唯一のクリア経路）**: `onLeaveNo()`が`doClear("refused")`を呼ぶ。`game_clear`のGA4パラメータは`{method:"refused", waitedMs}`（`waitedMs`は一時停止分を除いた実測待機時間）。
- **「見る」→狂死ルートへ合流**: `onLeaveYes()`が`playNextThreadSequence("dialog")`を呼ぶ。`route="dialog"`は`route="decoy"`と同じフル導入（来たね→来た来た→きたきたきたきた→あなた化→崩壊）で再生する（プレイヤー自身が「見に行く」を選んだ末路という点で decoy と同じ動機のため）。GA4は既存の`bad_end{route:"decoy"|"flow"|"dialog", flowAccum}`にそのまま合流。
- **「次スレへ」ボタンは独立して常時有効**: ダイアログの表示有無に関係なく、`#decoy`タップは従来どおり即座に`hardReset()`→`playNextThreadSequence("decoy")`（ダイアログが出ていれば同時に閉じる）。
- **セーフゾーン**: `.leave-dialog`はフルスクリーンの`.overlay`ではなく固定位置の小カードとして実装し、`#decoy`を覆わない（囮はダイアログ表示後も独立してタップ可能）。上部70%内（`top:44%`、600px高では`top:40%`に圧縮）。
